...
def save_sites_cache(
    ...
    *,
    ...
    conn.execute(
        """
        INSERT INTO facilities_land (code, sites_json, cache_version, pdf_size, pdf_mtime, updated_at)
        VALUES (?, ?, ?, ?, ?, ?)
        ...
        """,
        (code, json.dumps(sites, ensure_ascii=False), cache_version, pdf_size, pdf_mtime, _now()),
    )
...
def save_resolve_cache(
    ...
    conn.execute(
        """
        INSERT INTO web_address_resolve (resolve_key, address, score, source_url)
        VALUES (?, ?, ?, ?)
        ...
        """,
        (resolve_key, entry["address"], entry["score"], entry["source_url"]),
    )
...
def load_resolve_cache(
    ...
    row = conn.execute(
        "SELECT address, score, source_url FROM web_address_resolve WHERE resolve_key = ?",
        (resolve_key,),
    ).fetchone()
    if row is None:
        return None
